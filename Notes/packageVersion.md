...
- so the most importent thing is that there is some part which is importent to know,why is it given in version - caret (^) :- this will only `increase` your `minor` and `patch` part. - tilde(~) :- this will increase only your `patch` part.
...
1. remember one thing it will update only inside `node modules` not a `package.json`.
        ...
        ---> it means your minor and patch part will only updated.
        - If you package.json look this
           {
                "express":"~1.2.1"
            }
...
1. package.lock.json -->
 - lock your all package version, it means If your `package.lock.josn` is avillabe
